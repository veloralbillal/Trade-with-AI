    ...
    <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-300 uppercase tracking-wider">
          ...
          <span>Select Signal Timer Duration</span>
        </div>
        <span className="text-[10px] text-indigo-400 font-mono font-bold">{selectedSeconds} Seconds</span>
      </div>
        ...
        {TIMER_OPTIONS.map((timer) => {
          ...
          return (
            <button
              ...
              className={`py-2 px-3 rounded-xl border text-center font-bold text-xs transition-all font-mono ${
                isSelected
