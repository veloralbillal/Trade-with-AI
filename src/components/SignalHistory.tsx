    ...
    <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-300 uppercase tracking-wider">
          ...
          <span>Trade Signal History</span>
        </div>
        <span className="text-[10px] text-slate-500 font-mono">{history.length} Trades Recorded</span>
      </div>
        ...
        <div className="flex flex-col gap-2 max-h-60 overflow-y-auto scrollbar-hide">
          {history.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/70 border border-white/5 text-xs font-mono"
              >
                <div className="flex items-center gap-2.5">
                  <div className={`p-1.5 rounded-lg ${isCall ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                ...
                <div className="text-right">
                  ...
                  {item.status === 'PENDING' ? (
                    ...
                    <span className="text-[10px] font-bold text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded flex items-center gap-1 justify-end">
                      <CheckCircle2 size={11} /> WIN (+${item.profit.toFixed(2)})
                    </span>
                  ) : (
                    <span className="text-[10px] font-bold text-rose-400 bg-rose-400/10 px-1.5 py-0.5 rounded flex items-center gap-1 justify-end">
                      <XCircle size={11} /> LOSS (-${item.amount.toFixed(2)})
                    </span>
                  )}
                </div>
              </div>
